...
public final class ModuleValidator {
  ...
  /** Qualified names of modules being compiled in this compilation (see {@link #knownModules}). */
  private final Set<String> knownModuleNames = new HashSet<>();
  ...
  public void addKnownModules(Collection<XTypeElement> modules) {
    ...
    for (XTypeElement module : modules) {
      knownModuleNames.add(module.getQualifiedName());
    }
  }

  /**
   * Returns {@code true} if {@code module} is being compiled in this compilation (i.e. Dagger will
   * generate factories for its binding methods), as opposed to coming from a dependency on the
   * classpath.
   */
  ...
  /**
   * Reports an error if {@code module} is from a dependency and is missing generated factories for
   * its {@code @Provides} / {@code @Produces} methods.
   *
   * <p>This typically means the Dagger compiler was not applied to the library that defines the
   * module. Factories for modules in the current compilation are generated by {@code
   * ModuleProcessingStep} and may not yet be visible via {@link XProcessingEnv#findTypeElement}, so
   * those modules are skipped.
   *
   * @see <a href="https://github.com/google/dagger/issues/5146">dagger#5146</a>
   */
  public void checkGeneratedFactoriesAvailable(
      ModuleDescriptor module, ValidationReport.Builder report) {
    XTypeElement moduleElement = module.moduleElement();
    if (isModuleFromCurrentCompilation(moduleElement)) {
      return;
    }
    for (ContributionBinding binding : module.bindings()) {
      if (binding.kind() != PROVISION && binding.kind() != PRODUCTION) {
        continue;
      }
      ...
      if (processingEnv.findTypeElement(factoryName) == null) {
        report.addError(
            String.format(
                "The Dagger factory type %s is missing. This usually means the Dagger compiler was"
                    + " not applied to the module %s where the binding is defined. Ensure that the"
                    + " dependency that contains that module runs the Dagger annotation processor"
                    + " (or KSP plugin).",
                factoryName.getCanonicalName(), moduleElement.getQualifiedName()),
            moduleElement);
        // One diagnostic per module is enough to point users at the root cause.
        return;
      }
    }
  }

...
for project in "${ANDROID_GRADLE_PROJECTS[@]}"; do
    ...
    AGP_VERSION=$AGP_VERSION_INPUT ./$project/gradlew -p $project buildDebug --stacktrace $CONFIG_CACHE_ARG
    AGP_VERSION=$AGP_VERSION_INPUT ./$project/gradlew -p $project testDebug  --continue --stacktrace $CONFIG_CACHE_ARG
done

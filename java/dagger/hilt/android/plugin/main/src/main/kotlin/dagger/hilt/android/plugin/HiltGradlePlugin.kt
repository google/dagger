...
import dagger.hilt.android.plugin.task.HiltCollectClassesTask
...
class HiltGradlePlugin @Inject constructor(private val providers: ProviderFactory) :
  ...
  private fun HiltPluginEnvironment.createHiltCompileOnlyConfiguration(
    ...
    val hiltCollectClassesTask =
      project.tasks.register(
        "hiltCollectClasses${variant.name.capitalize()}",
        HiltCollectClassesTask::class.java,
      )

    variant.artifacts
      .forScope(ScopedArtifacts.Scope.PROJECT)
      .use(hiltCollectClassesTask)
      .toGet(
        type = ScopedArtifact.POST_COMPILATION_CLASSES,
        inputJars = HiltCollectClassesTask::classesJars,
        inputDirectories = HiltCollectClassesTask::classesDirectories,
      )
    ...
    testedVariant
      ...
      ?.toGet(
        ...
        inputJars = HiltCollectClassesTask::testedClassesJars,
        inputDirectories = HiltCollectClassesTask::testedClassesDirectories,
      )
    ...
    return project.configurations.create("hiltCompileOnly${variant.name.capitalize()}").apply {
      ...
      project.dependencies.apply {
        add(name, project.files(hiltCollectClassesTask.map { it.classesJars }))
        add(name, project.files(hiltCollectClassesTask.map { it.classesDirectories }))
        add(name, project.files(hiltCollectClassesTask.map { it.testedClassesJars }))
        add(name, project.files(hiltCollectClassesTask.map { it.testedClassesDirectories }))
      }
    }
  }

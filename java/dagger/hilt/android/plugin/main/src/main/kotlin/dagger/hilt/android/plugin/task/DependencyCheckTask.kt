...
abstract class DependencyCheckTask : DefaultTask() {
  ...
  @get:Input
  abstract val compileConfigurationName: Property<String>

  @get:Input
  abstract val processorConfigurationNames: ListProperty<String>
  ...
  fun check() {
    val runtimeDependencies =
      project.configurations.getByName(compileConfigurationName.get()).getDependenciesIds()
    if (!runtimeDependencies.contains(LIBRARY_GROUP to "hilt-android")) {
      error(getMissingDepMsg("$LIBRARY_GROUP:hilt-android"))
    }

    val processorDependencies = processorConfigurationNames.get()
  ...
  private fun Configuration.getDependenciesIds() = incoming
    .dependencies
